...
Original file is located at
    https://colab.research.google.com/drive/1Y7LD-CaLRqfeRp9FLM14EY53YDX8zpS-
"""
...
def gru_model(timestamp,features):

    inp= Input(shape=(timestamp,features,)) #BATCH,TIMESTAMP,FEATURES
    x= tf.keras.layers.GRU(128,return_sequences=False,name= 'GRU')(inp)#batch,timestamp,32
...
#     out= tf.keras.layers.TimeDistributed(dense)(x)
    model= Model(inp,out)
    return model
model= gru_model(input_width,len(Features))
model.summary()
